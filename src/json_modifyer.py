...
def select_latest_json_file(path):
...
def file_saver(path, json_name, data):
    create_dir(path)
    dest_name = json_name.replace(
        "json_structure_analyzed/analyzed_structure", path + "/updated_structure"
    )
    print(dest_name, "\n", path)
